...
Try {
	if ((Get-Item $testRoot$dirName) -is [System.IO.DirectoryInfo])
	{
		...
		if ($langName.StartsWith("udl-"))
		{
			...
			..\..\bin\mikepad++.exe -export=functionList -udl="`"$langName"`" $testRoot$dirName\unitTest | Out-Null
		}
		else
		{
			..\..\bin\mikepad++.exe -export=functionList -l"$langName" $testRoot$dirName\unitTest | Out-Null
		}
