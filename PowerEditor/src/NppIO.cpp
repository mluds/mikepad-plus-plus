...
bool Notepad_plus::doReload(BufferID id, bool alert)
{
	if (alert)
	{
		int answer = _nativeLangSpeaker.messageBox("DocReloadWarning",
			_pPublicInterface->getHSelf(),
			TEXT("Are you sure you want to reload the current file and lose the changes made in Mikepad++?"),
			TEXT("Reload"),
...
bool Notepad_plus::doSave(BufferID id, const TCHAR * filename, bool isCopy)
{
	...
	else if (res == SavingStatus::SaveOpenFailed)
	{
		...
		{
			// try to open Mikepad++ in admin mode
			bool isSnapshotMode = NppParameters::getInstance().getNppGUI().isSnapshotMode();
			if (isSnapshotMode) // if both rememberSession && backup mode are enabled
			{                   // Open the 2nd Mikepad++ instance in Admin mode, then close the 1st instance.
				int openInAdminModeRes = _nativeLangSpeaker.messageBox("OpenInAdminMode",
				_pPublicInterface->getHSelf(),
				TEXT("This file cannot be saved and it may be protected.\rDo you want to launch Mikepad++ in Administrator mode?"),
				TEXT("Save failed"),
				MB_YESNO);
				...
				if (openInAdminModeRes == IDYES)
				{
					...
					if (shellExecRes < 32)
					{
						_nativeLangSpeaker.messageBox("OpenInAdminModeFailed",
							_pPublicInterface->getHSelf(),
							TEXT("Mikepad++ cannot be opened in Administrator mode."),
			...
			{    // open only the file to save in Mikepad++ of Administrator mode by keeping the current instance.
				int openInAdminModeRes = _nativeLangSpeaker.messageBox("OpenInAdminModeWithoutCloseCurrent",
				_pPublicInterface->getHSelf(),
				TEXT("The file cannot be saved and it may be protected.\rDo you want to launch Mikepad++ in Administrator mode?"),
				TEXT("Save failed"),
				MB_YESNO);
	...
	if (!fn.empty())
	{
		...
		else		//cannot save, other view has buffer already open, activate it
		{
			_nativeLangSpeaker.messageBox("FileAlreadyOpenedInNpp",
				_pPublicInterface->getHSelf(),
				TEXT("The file is already opened in Mikepad++."),
				TEXT("ERROR"),
				MB_OK | MB_ICONSTOP);
			switchToFile(other);
			return false;
