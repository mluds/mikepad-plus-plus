	...
	if (posFind == -1) //no match found in target, check if a new target should be used
	{
		...
		{ // not found
			...
			if (NotIncremental == pOptions->_incrementalType) //incremental search doesnt trigger messages
			{
				...
				// if the dialog is not shown, pass the focus to his parent(ie. Mikepad++)
...
void Finder::copyPathnames()
{
	...
	if (!toClipboard.empty())
	{
		...
		{
			assert(false);
			::MessageBox(NULL, TEXT("Error placing pathnames into clipboard."), TEXT("Mikepad++"), MB_ICONINFORMATION);
		}
	}
}
...
void Finder::copy()
{
	...
	if (!toClipboard.empty())
	{
		...
		{
			assert(false);
			::MessageBox(NULL, TEXT("Error placing text in clipboard."), TEXT("Mikepad++"), MB_ICONINFORMATION);
		}
	}
}
