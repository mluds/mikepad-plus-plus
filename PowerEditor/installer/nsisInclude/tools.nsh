...
Function LaunchNpp
  ...
  Exec '"$WINDIR\explorer.exe" "$INSTDIR\mikepad++.exe"'
  ...
  ${ForEach} $R1 1 5 + 1				; Loop to find opened Npp instance
	...
	IntCmp $R0 0 NotYetExecuted
		...
		Exec '"$INSTDIR\mikepad++.exe" "$INSTDIR\change.log" '
		${Break}
	NotYetExecuted:
		Sleep 1000
  ${Next}
...
!macro CheckIfRunning un
	Function ${un}CheckIfRunning
		...
		IntCmp $R0 0 NotRunning
			System::Call 'kernel32::CloseHandle(i $R0)'
			MessageBox MB_RETRYCANCEL|MB_DEFBUTTON1|MB_ICONSTOP "Cannot continue the installation: Mikepad++ is running.\
			          $\n$\n\
                      Please close Mikepad++, then click ''Retry''." IDRETRY Retry IDCANCEL Cancel
			Retry:
				Goto Check
...
Function ExtraOptions
	...
	${NSD_CreateCheckbox} 0 80 100% 30u "Don't use %APPDATA%$\nEnable this option to make Mikepad++ load/write the configuration files from/to its install directory. Check it if you use Mikepad++ in a USB device."
...
Function preventInstallInWin9x
	...
	StrCmp $WinVer "95" 0 +3
		MessageBox MB_OK "Mikepad++ does not support your OS. The installation will be aborted."
		Abort
	...
	StrCmp $WinVer "XP" 0 xp_endTest ; XP
		MessageBox MB_YESNO "This version of Mikepad++ doesn't support Windows XP. The installation will be aborted.$\nDo you want to go to Mikepad++ download page for downloading the last version which supports XP (v7.9.2)?" IDYES xp_openDlPage IDNO xp_goQuit
xp_openDlPage:
...
xp_endTest:
	...
	StrCmp $WinVer "2003" 0 ws2003_endTest ; Windows Server 2003
		MessageBox MB_YESNO "This version of Mikepad++ doesn't support Windows Server 2003. The installation will be aborted.$\nDo you want to go to Mikepad++ download page for downloading the last version which supports this OS?" IDYES ws2003_openDlPage IDNO ws2003_goQuit
ws2003_openDlPage:
		ExecShell "open" "https://notepad-plus-plus.org/downloads/v7.9.2/"
ws2003_goQuit:
		Abort
...
Function writeInstallInfoInRegistry
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\mikepad++.exe" "" "$INSTDIR\mikepad++.exe"
	...
	!endif
	WriteRegStr HKLM "${UNINSTALL_REG_KEY}" "Publisher" "Mikepad++ Team"
	...
	WriteRegStr HKLM "${UNINSTALL_REG_KEY}" "DisplayIcon" "$INSTDIR\mikepad++.exe"
